...
typedef CF_OPTIONS(CFOptionFlags, CFURLEnumeratorOptions) {
    ...
    kCFURLEnumeratorGenerateRelativePathURLs API_AVAILABLE(macos(10.15), ios(13.0), bridgeos(4.0), watchos(6.0), tvos(13.0)) = 1UL << 6, /* The directory enumerator always generates file path URLs relative to the directory URL. This can reduce the size of each URL object returned during enumeration. */
};
